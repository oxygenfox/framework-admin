...
class Dashboard extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->post('keyword')) {
            $data['all_user']  = $this->dashboard->searchUserData();
        }
    ...
    public function role()
    {
        ...
        $data['role'] = $this->dashboard->getUserRoleAll();
    ...
    public function roleaccess($role_id)
    {
        ...
        $data['role'] = $this->dashboard->getUserRoleById($role_id);;
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('dashboard/role-access', $data);
        $this->load->view('templates/footer', $data);
    }
    ...
    public function editrole($role_id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('dashboard/edit-role', $data);
            $this->load->view('templates/footer');
        } else {
            $role_name = $this->input->post('role');
            $user_role = $this->db->get_where('user_role', ['role' => $role_name]);
            if ($user_role->num_rows() < 1) {
                ...
                redirect('dashboard/editrole/' . $role_id);
            }
        }
    }
    ...
    {
        $role = $this->dashboard->getUserRoleById($role_id);
